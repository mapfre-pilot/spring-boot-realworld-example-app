...
| Capa | Objetos |
|---|---|
| Catálogo | Record `SMK Test` (tabla `smk_test`; fuente: Connected System `SMK Database AWS`, Aurora PostgreSQL, esquema `smk_pruebashumo`): codigo, nombre, categoria (PLATAFORMA/DB/S3/HTTP/APIGW), sistema, criticidad, activo, motivoInactivo, origenAlta, orden, umbralMs |
| Ejecuciones | Record `SMK Ejecucion` (`smk_ejecucion`; fuente: Connected System `SMK Database AWS`): estado (EN_CURSO/OK/WARN/KO), origen, motivo, lanzadoPor, totales, duracionMs |
| Programaciones | Record `SMK Programacion` (`smk_programacion`; fuente: Connected System `SMK Database AWS`): estado (PENDIENTE/LANZADA/CANCELADA), fechaProgramada, alcance y enlace a la ejecución resultante |
...
Ver `deploy/GUIA_DESPLIEGUE.md`. Resumen: los cuatro record types SMK usan el Connected System `SMK Database AWS` sobre Aurora PostgreSQL, esquema `smk_pruebashumo`; el CS y el esquema/usuario deben existir en cada entorno. Para las tablas y la carga inicial del catálogo, la ruta recomendada es Liquibase contra la URL PostgreSQL del entorno destino. Pasos: exportar paquete → ejecutar Liquibase → importar → configurar `SMK_GRUPO_ADMINISTRADORES` por entorno → grupos → site *Catálogo* → **Sincronizar catálogo** → *Ejecutar* → baseline → activar/desactivar por entorno. `deploy/01_ddl_smk.sql` y `deploy/02_catalogo_smk.sql` se mantienen como fallback legado de MariaDB/`jdbc/Appian`.

## Evolución del catálogo (detectar conexiones / BD nuevas)
- **Fuente de verdad**: regla `SMK_catalogoBase` (124 pruebas, incluidas 12 `INT_SIN_CS`). `SMK_catalogoPendiente` devuelve las que faltan en `smk_test`; el botón *Sincronizar catálogo* las inserta (nunca actualiza ni borra las existentes). Así una prueba nueva se añade una vez en DEV y llega a TEST/PRO con el paquete.
