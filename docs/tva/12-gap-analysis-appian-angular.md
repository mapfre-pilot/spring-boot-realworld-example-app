...
| R2C_CAPTURA / R2C_PRECIOS | `TVA_Pantalla_R2C_*`, `TVA_R2C_*` | `r2c-captura/precios.container` | `PM TVA_R2C_PM_CAPT_SIGUIENTE`, `PRECIOS_RECALCULAR`, `PRECIOS_CONTRATAR` | P | Validación `TVA_SimuladorRentas_Captura_Validacion` implementada vía `errores_rentas_captura` + `validar-seccion` en caja `R2C_CAPTURA` (dos tomadores obligatorios, % participación, importe/periodicidad de la renta), botón Recalcular. |
...
| RGPD, Digitalización DNI, Cesión derechos, Intervinientes, Notas (pop-ups) | tomador / solicitud | P (RGPD/DNI/test integrados vía Appian Embedded; el resultado RGPD/DNI se verifica vía `cmp-respuesta-componente` (one-shot), no se confía en el `submit`; cesión/intervinientes pendientes) |
...
| 2 | Tomador | Secciones datos personales / domicilio / contacto / FATCA / representante legal con validaciones §12.4.4, panel de requisitos (RGPD, DNI, test conveniencia), plegado con validez | hecho (pop-ups RGPD/DNI/test conveniencia integrados vía Embedded Interfaces con verificación real de resultado; la Captura DNI ya funciona desde esta máquina — origen whitelisted, tarea real creada en TEST) |
