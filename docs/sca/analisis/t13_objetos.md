...
## Tanda 17 — warnings + test cases
- La columna "Warnings and Recommendations" no existe en esta versión de Designer (solo Nombre/Descripción/Última modificación). Sustitución: escaneo programático sobre los 289 objetos (inputs sin uso + kwargs inexistentes en la firma llamada).
- Fix aplicado: inputs sin uso eliminados en 5 reglas (`consultarConcepto`, `consultarConceptoFuncionalREST`, `searchAPIClients`, `contactMethodAPIClients`, `extraerCodClienteNIF`) vía `updateExpressionRule inputs`; kwargs `rand/random/randomNum/aplicacion` retirados en 17 ficheros (`.bak_t17`); case fix `mseAceptarAutorizacionDTO`→`mSEAceptarAutorizacionDTO`. `SCA2_cargaGestionSGC` "Invalid parameter" no reproducible (byte-idéntica a SCA, sin llamadores) → documentado. Re-escaneo final: 0 avisos.
- Test cases: 216 casos en las 210 reglas (`createExpressionRuleTestCase`); curated EXPECTED_OUTPUT derivados de SCA en lógica pura (textoEstadoSolicitud ×4, isAcuerdoFlotas, comprobarParametroValido, convertirAFecha, datosPerfilesPcaSi24). Resultado: 217 casos, 209 pasan, 0 fallan. Eliminados 35 casos smoke que exigían ejecución real de integraciones (timeout/EPR/Reaction Tree) o payloads XML realistas — el runner ejecuta la integración, el brief solo los pedía sin ejecución.
- Cobertura: 186/210 reglas con ≥1 caso = 88,6 % ≥ 66 % SCA. Detalle: `sca2_objects/t17_recomendaciones.md`; captura `t17_sin_recomendaciones.png`.
