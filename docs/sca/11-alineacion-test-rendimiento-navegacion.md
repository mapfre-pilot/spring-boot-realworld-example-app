...
### §8.z7 — Barreda UI: fecAnulacion, posponer, etiquetas de tarea, compañía

| Item | Causa | Fix |
|---|---|---|
| Detalle "Fecha anulación" `20/03/4833` (15787530) | `SCA2_construirContextoAlta` dividía `left(tostring(fecAnulacion),10)` por `-`; con entrada dd/MM/yyyy producía `"//01/02/2027"`. | Regla: si no hay 3 partes por `-`, se guarda el valor tal cual (acepta ISO `yyyy-MM-dd` y `dd/MM/yyyy`). Deploy + re-GET (`local!fs` presente). Backfill: filas Datos id=9 → `01/02/2027`; id=1 ISO → `04/03/2027`. `cargarSolicitud` ya devuelve `fecanulacion: "01/02/2027"`. |
| POSPONER 15787528 sin efecto | El click de prueba fue previo al deploy del `a!startProcess` único (el antiguo `saveInto` encadenaba 2 smart services → error antes de ejecutar). | `testProcessModel SCA2 CMD Posponer` (15787528, tarea 4, fechaDietario `2026-10-02`, motivo) → **COMPLETED**; Solicitud: `estadoTarea=POSPUESTA`, `caducidadTarea=2026-10-02`, `fechaDietario=2026-10-02`, `contadorPosponer=1`. La UI ya guarda `ri!onCompletar`→`local!hecho` en `onSuccess` = banner "Acción registrada" y vuelta (equivalente al `a!save(ri!numPoliza, null)` de SCA). |
...
### §8.z12 — Autorización paridad + sol.datos + catalogación + timing Alta

- **Fecha anulación autorizada** (FueraNorma/Autorización): `local!fechaAnulacion` leía `sol.datos.fecAnulacion` — inexistente (path y case). Ahora `sol.datosCompletosSolicitud.datosSolicitud.fecanulacion` con parse dd/MM/yyyy→date (split+`date()`). El campo ya tenía `required:true` — ahora llega poblado.
- **`sol.datos` roto globalmente**: `cargarSolicitud` no exponía la clave `datos`. Reestructurado: `local!datosSolMap` reutilizado como `datosCompletosSolicitud.datosSolicitud` y como `sol.datos` (alias). Consumer `canalEntrada`→`canalentrada` corregido en AccAdm. (a!map rechaza keys case-insensitive duplicadas — no se pueden aliar keys con otro case.)
- **Catalogación vacía**: PM Alta nodo 7 (Contexto) leía `datosContexto.datosSolicitud.catalogacion` (key inexistente) → `pv!catal=null` → columna `catalogacion` vacía. Ahora lee `datosCod.codTpCatalogacion` (código) → `determinarCatalogacionRenovada` → guardado en Datos Solicitud (la UI lo muestra vía `SCA2_TXT_TIP_CATALOGACION_PCA[code]` → "A FECHA"). Backfill: Datos id=11 (15787539) e id=12 (15787543) → catalogacion "4" (A FECHA).
- **Timing Alta ~60s**: la cadena completa es ~8s en records (Transicion timestamps 11:23:37→45). El tiempo restante es latencia PRE de `generarStudAnul` + evaluación client-side de `construirContextoAlta` (~2.2s medidos) — sin cambio estructural: Start Decidir ya es asíncrono y la Solicitud se escribe antes del End. Mantener `isSynchronous:true` es necesario para obtener pv.idSolicitud en onSuccess.

### §8.z13 — Posponer UI (Datetime→Date), OK→acción (refreshAlways), fecha autorizada

- **POSPONER UI sin efecto**: los `testProcessModel` usaban `fechaDietario:"2026-10-02"` (Date) y funcionaban, pero la UI pasa `obtenerCaducidadNivel(...)` = `now()+7` (**Datetime**) al PV `Date` del PM → `a!startProcess` fallaba en el arranque → sin CORRECTO, tarea intacta. Fix: `todate(...)` en el saveInto de POSPONER de las 4 interfaces (AccAdm, CA Opciones, FueraNorma, VERTI). Seguridad descartada (JJGONZ2 inició CMD Alta sin problemas desde UI — mismo folder/security; re-GET del código desplegado correcto).
- **OK tras alta caía en Detalle, no en acción**: `local!tareaNueva` solo se referenciaba en el saveInto del OK → local lazy nunca evaluado → fallback a Detalle. Añadido `refreshAlways:true` al `a!refreshVariable` para que el polling a Tarea PENDIENTE se ejecute de verdad.
- **"Fecha de anulación autorizada" sin asterisco**: el campo ya tenía `required:true` y `validationGroup:"FechaDeAnulacion"` (re-GET verificado — asterisco/validación correctos); lo que faltaba era el valor, ya corregido en §8.z12.

### §8.z14 — POSPONER UI: causa raíz real verificada en navegador

- **Cadena de fallos encontrada en vivo (15787543, JJGONZ2, Chrome)**:
  1. Card de debug con `a!richTextItem(style:"ERROR")` — "ERROR" no es un estilo válido → rompía toda la interfaz al evaluar (mi propia instrumentación; corregido a `"STRONG"`).
  2. **`cons!SCA2_PM_CMD_POSPONER` era tipo TEXT** (valor=uuid) en vez de PROCESS_MODEL → `a!startProcess` entraba por `onError`. Constante corregida a PROCESS_MODEL (PUT + re-GET).
  3. **Doble smart service por evaluación**: `SCA2_insertarObservaciones` (integración) dentro del saveInto, y `SCA2_guardarTrazabilidad` dentro del `onSuccess` — el onSuccess cuenta en la misma evaluación → "Attempted to run a second smart service" en TODOS los botones con ese patrón (POSPONER ×4, FINALIZAR/CANCELAR de AccAdm/CA Opciones/FueraNorma, CONTINUAR de VERTI). El PM sí llegaba a arrancar (Tarea 9 quedó pospuesta: dietario 2026-10-02, desasignada) pero el onSuccess abortaba → sin CORRECTO.
- **Fix desplegado (4 interfaces)**: `insertarObservaciones`/`guardarTrazabilidad` eliminados de saveInto/onSuccess — el CMD Posponer ya escribe observación ("Solicitud pospuesta."+motivo), y CMD CompletarAccion ya tiene nodo Guardar Trazabilidad. `onSuccess` queda solo con `a!save(ri!onCompletar, true)`; `onError` guarda `local!debugPosponer`/`local!errorPosponer` con card visible en AccAdm.
- **Pérdida documentada**: la traza técnica (record Trazabilidad) ya no se escribe en posponer desde UI — el PM CompletarAccion la cubre para finalizar/cancelar; para Posponer/Mecanizar se puede añadir un nodo en el PM si se requiere.
- **Pendiente**: botones POSITIVO/NEGATIVO de CA Opciones encadenan varias integraciones (`insertarObservaciones`+`modificarCatalogacion`+`altaSGO*`) en un solo saveInto → mismo límite; requiere refactor mayor (mover a PM o dividir en pasos).
- **Verificado en navegador**: POSPONER → confirm "Se va a posponer la solicitud de anulación" → CORRECTO "Los datos se han guardado correctamente" → ACEPTAR → Buscador. Tarea vuelta al grupo (sin asignadoA) — paridad SCA.

### §8.z15 — Cierre de "second smart service" en todas las acciones (25/09)

- **PM `SCA2 CMD CompletarAccion` +3 nodos** (via REST lcp-api, `createProcessModelNode` MCP fallaba `NoneType is not iterable`; hay que crear nodos hijo antes que el XOR que los referencia):
  - Nodo 300 XOR "Llamada cierre?": `finalizadoCA=true`→301 / `operacion="Autorizacion"`→302 / default→200.
  - Nodo 301 Call Integration `SCA2_finalizarContraAnulPcaIntegracion` (IContraAnularPCA), `consulta = index(pv!resultado,"finalizarCAPca",null)`.
  - Nodo 302 Call Integration `SCA2_aceptarAutorizacionIntegracion` (IGestionarAutorizacionesPCA), `consulta = 'type!{...}aceptarAutorizacion'(MSEAceptarAutorizacion: index(pv!resultado,"mSEAceptarAutorizacion",null))`.
  - Nodo 7 (Write PDTE_FINALIZAR) recableado 200→300. `validateDesignObject` → `hasErrors:false`.
- **Interfaces**: `SCA2_ContraAnulacionOpciones` FINALIZAR pasa `finalizadoCA:true` + map `finalizarCAPca` en `resultado` (el PM llama Core7); `SCA2_AnulacionFueraNormaPrincipal` FINALIZAR pasa `operacion:"Autorizacion"` + `mSEAceptarAutorizacion`. `onSuccess` solo `a!save(ri!onCompletar,true)` en todos los botones.
- **POSITIVO/NEGATIVO**: el "second smart service" reportado era el mismo fallo de POSPONER ya corregido — sus integraciones (`modificarCatalogacionCodCiaCntrAnul`, `altaSGO*`, `guardarEjecuArg`) son **HTTP puras (no son smart services)** → no cuentan en el límite; no requieren refactor. Integración ≠ smart service; el límite solo aplica a `a!startProcess`/`a!writeRecords`/`a!deleteRecords` (y reglas que los envuelven).
- **Hallazgos residuales corregidos**: `SCA2_MecanizacionPrincipal` aún llamaba `SCA2_guardarTrazabilidad` (a!writeRecords) en onSuccess de MECANIZAR y CANCELAR → eliminados (el PM escribe la traza); `local!searchAPI`/`local!checksPECA` quedaron unused → declarations eliminadas. `SCA2_BandejaErrores` Relanzar tenía `onSuccess: rule!SCA2_relanzarError` (a!writeRecords → mismo error) → sustituido por `local!idsRelanzadas` (marcado local que oculta el enlace tras el click; el record Error queda PENDIENTE — pérdida documentada, marcar vía PM si se requiere). Ojo: `onSuccess` dentro de `a!startProcess` anidado en `a!save` en columna de grid no acepta `a!save` → el guardado va en el saveInto.
- **Asterisco "Fecha de anulación autorizada"**: SCA usa `a!dateField` + `required:true` idéntico; en la site page la plataforma no renderiza el `*` automático → añadido literal `" *"` al label (paridad visual; el selector de oficina ya usa literal). Verificado en navegador: label "Fecha de anulación autorizada *" + valor 25/09/2026 poblado.

### §8.z16 — Relanzar marca el Error RELANZADO (nuevo PM)

- **Nuevo PM `SCA2 CMD MarcarErrorRelanzado`** (`0000f073-49f2-8000-d0e7-7f0000014e7a`): Start→Write "Marcar error RELANZADO" (Error `estado=RELANZADO`, `resueltoPor=pp!initiator`, `fechaResolucion=now()`, solo si `idError` no nulo)→XOR "Comando?"→6 nodos Start Process asíncronos (Decidir/CrearAccion/CompletarAccion/Finalizar/Mecanizar/Caducar, con `{idSolicitud, idTarea}`)→End. PVs: idError (Integer), idSolicitud, idTarea, comando (Text, params).
- **Constante `SCA2_PM_CMD_MARCAR_ERROR_RELANZADO`** (PROCESS_MODEL) creada; `SCA2_BandejaErrores` Relanzar ahora hace un único `a!startProcess` al PM con `{idError, idSolicitud, comando}` — sin segundo smart service (el write va dentro del PM).
- **Notas API**: `createProcessModel` MCP devuelve 401 → el PM se creó por REST (requiere nodes Start/End inline, `appUuid`, `errorAlertGroupName`="SCA2 Administrators" — *no* uuid). XOR: conexiones y condiciones deben añadirse juntas en el PUT (si no, "output nodes that do not exist"); PUT con >2 conexiones sin decisión coherente da "JSONArray[0] is not a JSONObject". createConstant type `PROCESS_MODEL` (enum en mayúsculas) directo.
- **Verificado**: validateDesignObject `hasErrors:false`; `testProcessModel{idError:8, idSolicitud:"15787510", comando:"X_NOEXISTE"}` → Error 8 marcado RELANZADO/resueltoPor=devin/fechaResolucion (rama default→End, sin lanzar CMD — la llamada síncrona agotó timeout pero el PM completó).
