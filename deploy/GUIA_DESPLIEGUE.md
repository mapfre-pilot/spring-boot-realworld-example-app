...
2. **Tablas**: al importar, Appian crea/actualiza `SMK_TEST`, `SMK_EJECUCION`, `SMK_RESULTADO` y `SMK_PROGRAMACION` en `jdbc/Appian` si el usuario del data source tiene permiso DDL (los record types tienen *Source configuration* con creación de tabla). Si no es así, ejecutar antes `01_ddl_smk.sql` en la BD de negocio del destino.
