...
|---|---|---|
| Record types | 4 | `SMK Test`, `SMK Ejecucion`, `SMK Resultado`, `SMK Programacion` (fuente `jdbc/Appian`, tablas `SMK_TEST`, `SMK_EJECUCION`, `SMK_RESULTADO`, `SMK_PROGRAMACION`) |
...
2. **Tablas**: la ruta recomendada es ejecutar Liquibase contra `jdbc/Appian` del entorno destino: `liquibase --changelog-file=deploy/liquibase/db.changelog-master.xml update`. Crea/actualiza `SMK_TEST`, `SMK_EJECUCION`, `SMK_RESULTADO` y `SMK_PROGRAMACION`, y carga el catálogo inicial sólo si `SMK_TEST` está vacía. `01_ddl_smk.sql` y `02_catalogo_smk.sql` se mantienen como fallback si no se puede usar Liquibase.
